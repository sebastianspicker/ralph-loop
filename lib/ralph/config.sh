...
validate_prd_structure() {
  ...
  [[ -n "$required_defaults_keys" && "$required_defaults_keys" != "null" ]] || fail "Could not read defaults.required from schema"
  [[ -n "$required_story_keys" && "$required_story_keys" != "null" ]] || fail "Could not read story.required from schema"
  [[ -n "$allowed_root_keys" && "$allowed_root_keys" != "null" ]] || fail "Could not read root properties from schema"
  [[ -n "$allowed_story_keys" && "$allowed_story_keys" != "null" ]] || fail "Could not read story properties from schema"
  [[ -n "$allowed_step_keys" && "$allowed_step_keys" != "null" ]] || fail "Could not read story_step properties from schema"

  jq -e \
    --argjson supported_modes "$SUPPORTED_MODES_JSON" \
    --arg created_regex "$CREATED_AC_REGEX" \
    ...
    "$PRD_FILE" >/dev/null || fail "Invalid prd.json structure or story constraints"
